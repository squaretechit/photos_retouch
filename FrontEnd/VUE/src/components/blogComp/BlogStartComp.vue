<script>
    ...
    export default {
        data: () => ({
            ...
            AllBlogs: ref(store.state.all_blogs_info),
            GoTop: false
        }),
        methods: {
            CheckBackToTopBtn(){
                ...
                <div class="col-lg-8">
                    <div v-for="blog in AllBlogs.slice().reverse()" :key="blog.id" class="blog-col">
                        <div class="blog-image">
                            <img loading="lazy" decoding="async" :src="BaseURL + blog.feature_picture" alt="{{ blog.title }}">
                        </div>
                        <div class="blog-col-content">
                            ...
                            <h2>
                                <router-link :to="{name:'SingleBlog', params:{blog_name: blog.url}}">{{ blog.title }}</router-link>
                            </h2>
                            <p v-html="blog.descriptions.slice(0,100) + '...'"></p>
                            <router-link class="btn btn-primary my-btn" :to="{name:'SingleBlog', params:{blog_name: blog.url}}">Read More <i class="fa fa-arrow-right"></i></router-link>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4">
